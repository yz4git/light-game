...
export function renderAreaClear(game, upgrades, {
  ...
  const body = `
    ...
    <fieldset class="upgrade-options">
      <legend>強化を1つ選んで、次のエリアへ</legend>
    ...
    <p class="clear-note">次のエリアで体力・アイテムが補充されます。</p>`;
  let leaving = false;
  modal('AREA CLEAR', `エリア ${game.stage + 1} クリア！`, body, [[
    `次のエリアへ（エリア ${nextStage + 1}） →`,
    () => {
      if (leaving) return;
